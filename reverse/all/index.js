...
const wordObject = {
    verb: VERBS,
    noun: NOUNS,
    adjectiveAdverb: [... ADJECTIVES_ADVERBS, ...EXPRESSIONS],
...
function start(wordsCount = allWords.length) {
    if (!started) {
        window.addEventListener('keyup', (event) => {
            ...
            if (words[counter.wordIndex].checkReverse(checked[0].value)) {
                ...
                [option1, option2, option3, option4] = takeFourWords(words[counter.wordIndex], words, counter, wordObject[words[counter.wordIndex].partOfSpeech]);
    ...
    words = shuffle(allWords.slice(allWords.length - wordsCount));
