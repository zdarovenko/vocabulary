...
document.getElementById('button').addEventListener('click', function () {
    ...
    if (Boolean(value) && value <= ADJECTIVES_ADVERBS.length) {
        start(value)
    } else {
        start();
    }
...
function start(wordsCount = ADJECTIVES_ADVERBS.length) {
    if (!started) {
        window.addEventListener('keyup', (event) => {
            ...
            if (words[counter.wordIndex].checkReverse(checked[0].value)) {
                ...
                [option1, option2, option3, option4] = takeFourWords(words[counter.wordIndex], ADJECTIVES_ADVERBS, counter);
    ...
    words = shuffle(ADJECTIVES_ADVERBS.slice(ADJECTIVES_ADVERBS.length - wordsCount));
